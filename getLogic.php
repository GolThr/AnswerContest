...
//mysql
$sql = "SELECT * FROM logic WHERE l_num='$paper_id'";
...
if ($result->num_rows > 0) {
    ...
    while ($row = $result->fetch_assoc()) {
        $logicSelect->id = $row['single_select_id'];
        $logicSelect->content = $row['content'];
        $logicSelect->A = $row['s_a'];
        $logicSelect->B = $row['s_b'];
        $logicSelect->C = $row['s_c'];
        $logicSelect->D = $row['s_d'];
    }
